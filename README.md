...
- **Remove** a game from your library from within the plugin, guarded by a confirmation dialog (the action is permanent).
- **Installed** column showing each duplicate's install state, plus an 'Only installed games' filter to find games accidentally installed on several platforms (issue #7).
